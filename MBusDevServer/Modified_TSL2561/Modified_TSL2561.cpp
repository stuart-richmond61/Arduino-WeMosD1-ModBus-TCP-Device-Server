 ...
 *   @section  HISTORY
 *
 *          Modified to remove Adafruit_Sensor layer for MBusDevServer use.
 *
...
/**************************************************************************/
bool Adafruit_TSL2561::getLuminosityNoBlock (uint16_t *broadband, uint16_t *ir)
{
  bool valid = false;
  ...
  if(!_tsl2561AutoGain)
  {
    return getDataNoBlock (broadband, ir);
  }
  ...
  do
  {
    ...
    if (!getDataNoBlock(&_b, &_ir)) return false;
...
*/
/**************************************************************************/
bool Adafruit_TSL2561::getDataNoBlock (uint16_t *broadband, uint16_t *ir)
{
  long delta;
  /* Enable the device by setting the control bit to 0x03 */
  if (!_getpending) {
  ...
  /* Wait x ms for ADC to complete */
  delta = millis() - _enablemillis;
  
  switch (_tsl2561IntegrationTime)
  {
    case TSL2561_INTEGRATIONTIME_13MS:
      if (delta < TSL2561_DELAY_INTTIME_13MS) return false;
      break;
    case TSL2561_INTEGRATIONTIME_101MS:
      if (delta < TSL2561_DELAY_INTTIME_101MS) return false; // KTOWN: Was 102ms
      break;
    default:
      if (delta < TSL2561_DELAY_INTTIME_402MS) return false; // KTOWN: Was 403ms
      break;
  }
  _getpending = false;
  /* Reads a two byte value from channel 0 (visible + infrared) */
...
*/
/**************************************************************************/
bool Adafruit_TSL2561::ReadLight()
{

  /* Calculate the actual lux value */
  getLuminosity(&_broadband, &_ir);
  _light = calculateLux(_broadband, _ir);
...
/**************************************************************************/
bool Adafruit_TSL2561::ReadLightNoBlock()
{
  /* Calculate the actual lux value */
  if (!getLuminosityNoBlock(&_broadband, &_ir)) return false;
  _light = calculateLux(_broadband, _ir);
