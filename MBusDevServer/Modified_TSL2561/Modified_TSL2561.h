...
#ifndef ADAFRUIT_TSL2561_
#define ADAFRUIT_TSL2561_
...
class Adafruit_TSL2561 {
 public:
  ...
  bool getLuminosityNoBlock (uint16_t *broadband, uint16_t *ir);
  uint32_t calculateLux(uint16_t broadband, uint16_t ir);
  
  bool ReadLight();
  bool ReadLightNoBlock();
  float LightReading() { return _light; };
  uint16_t BroadbandReading() { return _broadband; };
  uint16_t irReading() { return _ir; };
 ...
 private:
  ...
  bool     getDataNoBlock (uint16_t *broadband, uint16_t *ir);
};
